...
class _MiniPathInitializer(object):
    ...
    def get_extern_bin_from_path_element_ending_with_runtime_arch(self, dir_found):
        ...
        if last_path_component != self.arch:
            output_str = ''.join(
                ('To call deployed MATLAB code on a {0} machine, you must run a {0} version of Python, ',
    ...
    def put_extern_bin_on_py_sys_path(self):
        ...
        if not path_elements:
            if self.system == 'Darwin':
                ...
                raise RuntimeError(
                    'On {0}, you must set the environment variable "{1}" to a non-empty string. {2}'.format(
...
def get_python_version():
    ...
    _supported_versions = ['3_8', '3_9', '3_10']
