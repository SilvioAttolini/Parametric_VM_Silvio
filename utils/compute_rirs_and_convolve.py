...
def compute_rirs_and_convolve(macro, params, source, room, array):
    fLen = params['f_len']
    tLen = params['t_len']
    ...
    for iSrc in range(source['N']):
        for aa in range(array['N']):
            for mm in range(array['micN']):
                ...
                rir_time, rir_freq = rir(params, room, source, array, (iSrc + 1), aa, mm, True)

                # Convolution with source signal
                impRespFrame = np.repeat(rir_freq.T, tLen, axis=1)
                current = source['STFT'][iSrc] * impRespFrame
                arraySTFT[aa, mm, :, :] += current

                if macro['COMPUTE_DIR_PATHS']:
                    # Direct path RIR, no walls
                    h_time, h_freq = rir(params, room, source, array, (iSrc+1), aa, mm, False)

                    # Convolution with source signal
                    hFrame = np.repeat(h_freq.T, tLen, axis=1)
                    current = source['STFT'][iSrc] * hFrame
                    arrayDirectSTFT[aa, mm, :, :] += current

                    # Debug
                    plots.debug_get_array_signals(macro, source, room, rir_time, arraySTFT, h_time, arrayDirectSTFT,
                                                  iSrc, aa, mm)
